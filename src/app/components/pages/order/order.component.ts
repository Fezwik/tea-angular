...
export class OrderComponent implements OnInit, OnDestroy {
  ...
  orderForm = new FormGroup({
    name: new FormControl('', [Validators.required, validation('^[A-Za-zА-Яа-яЁё]+$')]),
    lastName: new FormControl('', [Validators.required, validation('^[A-Za-zА-Яа-яЁё]+$')]),
    phone: new FormControl('', [Validators.required, validation('^\\+?\\d{11}$')]),
    country: new FormControl('', [Validators.required, validation('^[A-Za-zА-Яа-яЁё\\s]+$')]),
    zip: new FormControl('', [Validators.required, validation('^\\d+$')]),
    address: new FormControl('', [Validators.required, validation('^[A-Za-zА-Яа-яЁё0-9\\s\\-\\/]+$')]),
  ...
  public createOrder(): void {
    this.error = false;
    this.success = false;
    this.subscriptionOrder = this.productService.makeOrder({
